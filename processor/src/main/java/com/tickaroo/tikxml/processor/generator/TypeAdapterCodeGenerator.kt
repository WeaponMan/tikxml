...
class TypeAdapterCodeGenerator(private val filer: Filer, private val elementUtils: Elements, private val typeUtils: Types, private val typeConvertersForPrimitives: Set<String>) {
    ...
    fun generateCode(annotatedClass: AnnotatedClass) {
        ...
        annotatedClass.childElements.filter { it.value is PlaceholderXmlElement }.forEach { xmlName, xmlElement ->
        ...
        val toXmlCodeBlock = generateToXmlMethod(annotatedClass, codeGenUtils).build()
    ...
    private fun generateFields(annotatedClass: AnnotatedClass, adapterClassBuilder: TypeSpec.Builder, customTypeConverterManager: CustomTypeConverterManager) {
        ...
        if (annotatedClass.childElements.any { it.value is PlaceholderXmlElement }) {
    ...
    private fun generateFromXmlMethod(annotatedClass: AnnotatedClass, codeGenHelper: CodeGeneratorHelper): MethodSpec.Builder {
        ...
        if (!annotatedClass.hasChildElements() && !annotatedClass.hasTextContent()) {
            ...
            builder.beginControlFlow("while(true)")
                    .beginControlFlow("if (\$L.hasElement())", reader)
                    .addCode(codeGenHelper.generateElementReadFlowControl(annotatedClass, targetClassToParseInto))
                    .nextControlFlow("else if (\$L.hasTextContent())", reader)
